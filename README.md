...
* MariaDB 10.6
* PHP-FPM 8.3 with the following extensions:
...
* OpenSearch: Poppular searche engine to use with third party search plugins
...
* opensearch: listening on port 9200 and 9600
* rabbitmq: listening on port 5672 and 15672
...
## SSL Configuration
You can generate a wildcard certificate by calling generatessc.sh folder in conf/certs. Be sure to be inside this folder as it is mapped inside nginx and it will be looking for certificates there. Then update the https/nginx.conf configuration file to match the certificate name.

# Varnish HTTPS support
Varnish by itself doesn't support HTTPS. So we end-up doing the Nginx (443) -> Varnish (80) -> Nginx (8080) sandwich.
...
## Thanks and credit
This repo is an assembly of configurations snippets grabbed all around, but mostly from PHP and MySQL docker builds.
